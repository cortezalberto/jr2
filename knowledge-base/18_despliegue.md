...
Integrador soporta multiples estrategias de despliegue segun el contexto: desarrollo local con Docker, desarrollo hibrido (Docker parcial + servicios manuales), DevContainer para VSCode, y un esquema de produccion que aun esta en fase de definicion. Este documento cubre cada estrategia en detalle.
...
> **Nota importante**: El despliegue a produccion NO esta completamente implementado. Lo que sigue es el plan arquitectonico.

### Estrategia Planificada

- **Backend**: Docker multi-stage build basado en `python:3.12-slim`. Imagen optimizada sin dependencias de desarrollo.
- **Frontends**: Build estatico con `npm run build` (Vite). Los archivos resultantes se sirven desde un CDN o servidor de archivos estaticos (nginx).
- **Base de datos**: PostgreSQL 16 gestionado (RDS, Cloud SQL, o similar).
- **Redis**: Instancia gestionada con persistencia AOF, 256MB maximo, politica de eviccion LRU.
...
|------------|--------|-------------|
| CI/CD Pipeline | No implementado | GitHub Actions o similar para build, test, deploy automatico |
| TLS/SSL | No configurado | Certificados para HTTPS. Requerido para cookies seguras |
| Log Aggregation | No implementado | Centralizacion de logs (ELK, CloudWatch, etc.) |
| Backups | No automatizado | Backup periodico de PostgreSQL |
| Escalado Horizontal | No implementado | Multiples instancias de backend detras de load balancer |
| Monitoreo | Parcial | web-vitals en frontend, pero falta APM en backend |
